<?php
/**
 * GitHub update checker for ART Portfolio.
 *
...
/**
 * Class Art_Portfolio_Updater
 */
class Art_Portfolio_Updater {

	const GITHUB_REPO = 'artbashlykov/art-portfolio';
	...
	public static function init() {
		$library = ART_PORTFOLIO_PLUGIN_DIR . 'vendor/' . 'plugin-' . 'update-checker' . '/' . 'plugin-' . 'update-checker.php';
		...
		$checker = call_user_func(
			...
			ART_PORTFOLIO_PLUGIN_FILE,
			ART_PORTFOLIO_ADMIN_MENU_SLUG
		);
	...
	public static function filter_api_request_options( $options ) {
		...
		$options['headers']['User-Agent'] = 'ART-Portfolio/' . ART_PORTFOLIO_VERSION;
	...
	private static function get_github_token() {
		...
		if ( defined( 'ART_PORTFOLIO_GITHUB_TOKEN' ) ) {
			$token = (string) ART_PORTFOLIO_GITHUB_TOKEN;
		}

		/**
		 * Filters GitHub token used to check ART Portfolio updates.
		 *
		 * @param string $token GitHub personal access token.
		 */
		$token = (string) apply_filters( 'art_portfolio_github_token', $token );
